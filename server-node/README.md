...
Node.js + Express + TypeScript + MongoDB (Mongoose). A port of the FastAPI
server in [`../server`](../server), matching its API exactly — same paths, same
request and response shapes, same error codes — so the existing React client
...
## Migrating from the FastAPI server

The two servers read and write the same collections with the same field names,
so they can run against one database and you can switch between them.

- **Passwords carry over.** bcrypt hashes written by passlib verify here, so
  seeded and registered accounts log in unchanged.
- **Issued tokens carry over**, provided `JWT_SECRET_KEY` is the same.
- **Calendar dates** (`starts_on`, `expires_on`, `profile.dob`,
  `gym_meta.joined_on`) are stored as `YYYY-MM-DD` strings here, where Beanie
  stored BSON dates. Documents written by the Python server are still read
  correctly; documents written here are `YYYY-MM-DD`. **Run both servers
  against the same database only if you have checked this against your
  data** — the safest path is to migrate fully rather than run them in
  parallel.
- Point the client at this server by setting `VITE_API_BASE_URL`, or leave the
  Vite dev proxy as it is: both servers listen on port 8000.

## Known issues

Behaviour carried over verbatim from the FastAPI server, including several
real defects, is catalogued in [`KNOWN_ISSUES.md`](KNOWN_ISSUES.md). Read it
before changing payment, OTP or subscription code.
