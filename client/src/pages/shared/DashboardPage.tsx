...
export default function DashboardPage() {
  ...
  return <div className="dashboard"><header className="mobile-header"><div className="brand"><span className="brand-mark"><Dumbbell size={17} /></span><span className="brand-text">FITCORE</span></div><div className="header-actions"><button className="icon-button" onClick={() => setDarkMode(!darkMode)} aria-label="Toggle theme">{darkMode ? <Sun size={18} /> : <Moon size={18} />}</button><button className="icon-button" onClick={handleLogout} aria-label="Sign out"><LogOut size={18} /></button></div></header><div className="app-body"><main className="member-main">{loading ? <div className="loading-state">Loading your gym space...</div> : <>{tab === "home" && <HomeView user={user} subscription={subscription} attendancePercent={attendancePercent} expirySoon={expirySoon} setTab={setTab} />}{tab === "plans" && <PlansView plans={plans} subscription={subscription} history={history} onChoosePlan={(plan) => { setSelectedPlan(plan); setTab("checkout"); }} />}{tab === "checkout" && selectedPlan && <CheckoutView plan={selectedPlan} onBack={() => setTab("plans")} onComplete={() => { setTab("home"); window.location.reload(); }} />}{tab === "attendance" && <AttendanceView subscription={subscription} />}{tab === "payments" && <PaymentsView payments={payments} />}{tab === "referrals" && <ReferralView referrals={referrals} />}{tab === "profile" && <ProfileView profile={profile} user={user} />}</>}</main><nav className="app-nav" aria-label="Main navigation">{([["home", Dumbbell, "Home"], ["plans", CreditCard, "Plans"], ["attendance", CalendarDays, "Track"], ["payments", BadgeIndianRupee, "Paid"], ["referrals", Share2, "Refer"], ["profile", UserRound, "Profile"]] as const).map(([key, Icon, label]) => <button className={tab === key ? "nav-item active" : "nav-item"} key={key} onClick={() => setTab(key)}><Icon size={19} /><span>{label}</span></button>)}</nav></div></div>;
}
...
function HomeView({ user, subscription, attendancePercent, expirySoon, setTab }: { user: NonNullable<ReturnType<typeof useAuthStore.getState>["user"]>; subscription: Subscription | null; attendancePercent: number; expirySoon: boolean; setTab: (tab: MemberTab) => void }) {
  return <section className="view-stack"><div className="welcome-row"><div><h1>Hey, {user.full_name.split(" ")[0]}.</h1></div><div className="avatar">{user.full_name.charAt(0)}</div></div>{expirySoon && <button className="renew-alert" onClick={() => setTab("plans")}><span><strong>Your plan ends in {subscription?.days_until_expiry} days.</strong><small>Renew now or explore another plan.</small></span><ChevronRight size={21} /></button>}<div className="home-split"><div className="hero-stat"><div className="stat-copy"><span>Current plan</span><strong>{subscription?.plan_snapshot.plan_name ?? "No active plan"}</strong><small>{subscription ? `${subscription.days_remaining} days left · expires ${dateLabel(subscription.expires_on)}` : "Choose a plan to start your membership"}</small></div><div className="progress-ring" style={{ "--progress": `${attendancePercent * 3.6}deg` } as React.CSSProperties}><span>{subscription ? subscription.days_remaining : 0}<small>left</small></span></div></div><div className="view-stack pulse-stack"><div className="section-heading"><div><p className="eyebrow">Your pulse</p><h2>This week</h2></div><button className="text-button" onClick={() => setTab("attendance")}>See details <ChevronRight size={16} /></button></div><div className="insight-grid"><article className="insight-card accent-card"><CalendarDays size={19} /><strong>{subscription?.days_used ?? 0}</strong><span>days used</span></article><article className="insight-card"><Sparkles size={19} /><strong>{user.membership_status === "active" ? "Active" : "Ready"}</strong><span>membership</span></article><article className="insight-card"><Users size={19} /><strong>Later</strong><span>rewards API</span></article></div></div></div><div className="quick-actions"><button onClick={() => setTab("plans")}>Explore plans <ChevronRight size={17} /></button><button onClick={() => setTab("referrals")}>Invite a friend <ChevronRight size={17} /></button></div></section>;
}
